...
var selectedHour,selectedMinute,selectedSecond;
var calenderType=0;
/* calenderType 0 with out Time 1 With Time*/
...
function CalendarControl() {
  ...
  function setDate(day, month, year) {
    if (dateField) {
      ...
      if (calenderType == 1){
          var timeString=document.getElementById('txtHour').value+":"+document.getElementById('txtMinute').value+":"+document.getElementById('txtSecond').value;
          dateField.value = dateString+" "+timeString;
          hiddenDateField.value=dateString1 +" "+timeString;
      }else{
...
function changeYearFromCombo(change) {
  ...
  function calendarDrawTable() {
    ...
    if (calenderType == 1){
        ...
        var time="<tr><th class='empty' colspan='7' style='padding: 3px;'>\n\
                    <input type='text' name='txtHour' id='txtHour' size='1' value='"+selectedHour+"' maxlength='2'>";
        time=time+" : <input type='text' name='txtMinute' id='txtMinute' size='1' value='"+selectedMinute+"' maxlength='2'>";
        time=time+" : <input type='text' name='txtSecond' id='txtSecond' size='1' value='"+selectedSecond+"' maxlength='2'></td>";
        table = table + time;
    }
  ...
  function show(field,field1) {
    ...
    if(dateField) {
      try {
        var tmpPatrs,timeParts,dateParts;
        var dateString = new String(dateField.value);
         if (calenderType == 1) {
            tmpPatrs=dateString.split(' ');
            dateParts = dateString.split("-",3);
            timeParts = tmpPatrs[1].split(":");
            selectedHour=parseInt(timeParts[0],10);
            selectedMinute=parseInt(timeParts[1],10);
            selectedSecond=parseInt(timeParts[2],10);
        }else{
...
function showCalendarControlWithTime(objTextField,argPastDay,argCurrentDay,argFutureDay,argCurrentDate,objHidden,argCalenderType){
  // textField.onblur = hideCalendarControl;
  futureAllowed=argFutureDay;
  pastAllowed=argPastDay;
  isCurrentDateAllow=argCurrentDay;
...
function getPreviousDateDifference(largCurrentDate,largPreviousDateValue){
         var re=/-/g;
         largCurrentDate=largCurrentDate.replace(re,'/');
         largPreviousDateValue=largPreviousDateValue.replace(re,'/');
         var ldatediff=DateDiff(largPreviousDateValue,largCurrentDate);
         return ldatediff;
}
